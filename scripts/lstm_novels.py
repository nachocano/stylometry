...
from novels import prepare_data, load_data, build_data, get_data
...
def train_lstm(train, valid, test, fold,
        ...
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if numpy.mod(uidx, dispFreq) == 0:
                    print 'epoch ', eidx, 'update ', uidx, 'cost ', cost, ' fold', fold
    ...
    print 'train ', train_err, 'valid ', valid_err, 'test ', test_err, ' for fold ', fold
    if saveto:
        numpy.savez(saveto, train_err=train_err,
                    valid_err=valid_err, test_err=test_err,
                    history_errs=history_errs, **best_p)
    print 'the code run for %d epochs in fold %s, with %f sec/epochs' % (
        (eidx + 1), fold, (end_time - start_time) / (1. * (eidx + 1)))
...
def main():
    ...
    for fold in folds:
        ...
        # let's don't care about the contexts for now... just get the average performance
    ...
    avgs = {}
    avgs["T"] = float(train_errs) / len(folds)
    avgs["V"] = float(valid_errs) / len(folds)
    avgs["Te"] = float(test_errs) / len(folds)

    print '%s,%s,%s\n' % (avgs['T'], avgs['V'], avgs['Te'])
