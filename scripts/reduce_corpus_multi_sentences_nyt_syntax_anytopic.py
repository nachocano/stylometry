...
def main():
  ...
  parser.add_argument('-if', '--input_folder', required=True) # any_text text
  parser.add_argument('-i', '--input_file_all', required=True) # all the corpus file
  ...
  for line in open(args.input_file_all).read().splitlines():
    ...
    if search_obj:
      ...
      if did in documents:
        ...
        print 'DOC-%s-%d-%d %s' % (did, fold, int(label), line_as_str)
