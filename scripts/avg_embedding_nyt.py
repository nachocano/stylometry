...
def main():
  ...
  p = re.compile(ur'^SENT-(\d{4}_\d{2}_\d{2}_\d+)-(\d+)-(\d)-(\d+)$')
  # did, fold, label, sentence_nr
  for line in open(args.input_file).read().splitlines():
    ...
    if search_obj:
      ...
      fold = int(search_obj.group(2))
      label = int(search_obj.group(3))
      sentence_nr = int(search_obj.group(4))
      nyt[(did, label)].append((fold, sentence_nr))
  ...
  for line in open(args.embeddings_file).read().splitlines():
    ...
    if search_obj:
      ...
      if (did, label) not in  new_nyt and len(nyt[(did, label)]) > 0:
        ...
        new_nyt_fold[(did, label)] = nyt[(did, label)][0][0]
  ...
  for elem in new_nyt:
    ...
    label = elem[1]
    fold = new_nyt_fold[(did, label)]
    of.write('%s %d %d %s %s %s %s\n' % (year, int(month), int(day), identifier, fold, label, embeddings_as_str))
  of.close()
